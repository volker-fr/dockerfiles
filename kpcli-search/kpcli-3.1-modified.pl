...
print "\n";
# search or be interactive
if (length($opts->{search})) {
    &cmd_find($opts->{search});
    &cli_close();
    exit;
}
...
sub MyGetOpts {
  ...
  my $result = &GetOptions(\%opts, "kdb=s", "key=s", "pwfile=s", "histfile=s",
	"help", "h", "readonly", "no-recycle", "timeout=i", "command=s", "search=s");
...
sub cmd_find($) {
  ...
  # show the single ones immediately
  foreach my $match (@matches) {
      cmd_show({ args => [ $match->{'full_path'} ] });
  }
}

####################
# own modified way to show it
####################
sub cmd_show($$) {
  my $params = shift @_;
  our $state;
  ...
  if ($ent->{path} =~ /\/Root\/Backup/ || $ent->{path} =~ /\/Root\/old/) {
    return;
  }
